...
function HomeStep() {
  const [cardData] = useState([
    ...
    },
    {
      ...
      image: 'Variant2.png',
      subTitle: '觀看預約須知',
      description: '預約前，請先仔細閱讀預約須知及價目表，確認您所需的服務項目。',
    },
    {
      title: 'Step 3',
      image: 'Variant3.png',
    ...
    },
    {
      ...
      image: 'Variant5.png',
      subTitle: '出發施作美甲',
      description: '按照預約的時間與地點，享受專業的美甲服務，讓您的指甲美美的吧！',
    },
  ]);
        ...
        <div className="row gap-8 px-4 mx-auto">
          {cardData.map((item, index) => (
            <div key={index} className="col-12 col-xl-2">
              <h3 className="fs-6 font-bold text-primary-02 mb-4">{item.title}</h3>
              <img
                src={item.image}
                alt={item.subTitle}
                className="w-[240px] h-[196px] object-cover rounded-lg"
              />
              <h4 className="fs-5 font-semibold text-gray-700 mt-6">{item.subTitle}</h4>
              <p className="fs-3 text-gray-600 mt-4">{item.description}</p>
            </div>
          ))}
        </div>
      </section>
